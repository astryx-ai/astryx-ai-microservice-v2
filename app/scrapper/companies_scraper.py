...
if __name__ == "__main__":
    ...
    print(f"Companies merged by ISIN: {len(companies)}")
    # pandas summary for null/non-null diagnostics
    try:
        import pandas as _pd
        df = _pd.DataFrame(companies)
        cols = [
            "company_name","nse_symbol","bse_code","bse_symbol","isin","industry","status","market_cap"
        ]
        existing = [c for c in cols if c in df.columns]
        print("\nNon-null counts:")
        print(df[existing].notna().sum())
        print("\nSample rows with BSE present:")
        has_bse = df[df["bse_code"].notna() | df["bse_symbol"].notna()]
        print(has_bse.head(10))
    except Exception as _e:
        print(f"pandas summary failed: {_e}")
